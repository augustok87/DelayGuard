...
- **Tests**: 2,385 passing of 2,410, 25 skipped, 0 failing across 120 suites — **stable**, verified 2026-07-29 by three consecutive identical full runs with coverage. A previously order-flaky gate (three runs, three different failures) was fixed by deleting five tautological suites that tested a hardcoded stub fixture instead of the real app; see [LAUNCH_PLAN.md](LAUNCH_PLAN.md) §6 R5. **The lesson worth keeping: a test that asserts a fixture returns its own constants is worse than no test — it costs gate reliability and buys no coverage.** Real route coverage lives in `tests/integration/server-app.test.ts` (boots the actual Koa app). Local CI gate: `npm test && npm run lint && npm run type-check && npm run build` (run from `delayguard-app/`).
