...
- **Phase**: 🚀 **Deployed to production 2026-07-28** — live at `https://delayguard-api.vercel.app`, `/health` green, full probe matrix passing. Phase 1 + launch workstreams WS-A..H complete; Phase 2.1.a–f all shipped. **2026-08-25: the first real delay email was delivered** from the production cron pipeline — the core loop is proven end-to-end. ⚠️ Reading that email opened two code blockers, so *"remaining work is not code"* is **no longer true**: see [LAUNCH_PLAN.md](LAUNCH_PLAN.md) **§7** for the critical path, then **§6 R17** (one send marks every alert on the order delivered — fix this first) and **R18**.
- **Tests**: 2,446 passing of 2,471, 25 skipped, 0 failing across 127 suites (2026-08-25). ⚠️ **Not certified stable** — [LAUNCH_PLAN.md](LAUNCH_PLAN.md) §6 R5 is open and was **re-diagnosed 2026-08-25**: the dominant mechanism is *wall-clock threshold assertions failing under machine load*, not order-dependent state leakage. It was reproduced on demand by running two `--coverage` suites concurrently. **Never run `npm test` in two sessions at once.** The tightest offenders: `App.test.tsx:70` (<100 ms), `input-sanitization.test.ts:405` (<120 ms), `tests/performance/load.test.ts`, and two thresholds inside `monitoring-service`. **The lesson worth keeping: a test that passes only because a setup-file mock hides a real network call is not testing what its name claims** — `checkExternalAPIs` would hit the public internet without `jest.setup.ts`. Local CI gate: `npm test && npm run lint && npm run type-check && npm run build` (run from `delayguard-app/`).
