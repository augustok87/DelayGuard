...
- **Tests**: 2,409 passing of 2,434, 25 skipped, 0 failing across 125 suites (2026-07-31). ⚠️ **Not certified stable** — the order-dependent flake of [LAUNCH_PLAN.md](LAUNCH_PLAN.md) §6 R5 is re-opened; it did not reproduce this session, but R5 requires three consecutive clean full runs before the gate is called trustworthy again. A related earlier flake was fixed by deleting five tautological suites that tested a hardcoded stub fixture instead of the real app. **The lesson worth keeping: a test that asserts a fixture returns its own constants is worse than no test — it costs gate reliability and buys no coverage.** Real route coverage lives in `tests/integration/server-app.test.ts` (boots the actual Koa app). Local CI gate: `npm test && npm run lint && npm run type-check && npm run build` (run from `delayguard-app/`).
