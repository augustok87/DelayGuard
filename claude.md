...
- **Tests**: 2,409 passing of 2,435, 25 skipped (verified 2026-07-29). ⚠️ **The suite is order-flaky** — three consecutive full runs each failed a *different* test, all of which pass in isolation. `maxWorkers: 1` (one shared process) plus Jest's duration-cached file ordering makes cross-suite state leakage nondeterministic; every flaking assertion lives in a suite importing the `tests/setup/test-server.ts` **stub fixture**, which covers no production code. See [LAUNCH_PLAN.md](LAUNCH_PLAN.md) §6 R5 — deletion is the recommended fix. **A commit may need one legitimate re-run; never `--no-verify`, and if the same test fails twice, it's real.** Local CI gate: `npm test && npm run lint && npm run type-check && npm run build` (run from `delayguard-app/`).
