...
- **Phase**: 🚀 **Deployed to production 2026-07-28** — live at `https://delayguard-api.vercel.app`, `/health` green, full probe matrix passing. Phase 1 + launch workstreams WS-A..H complete; Phase 2.1.a–f all shipped. **2026-08-25: the first real delay email was delivered** from the production cron pipeline. **2026-08-26: the pipeline's correctness blockers are closed** — R17 (one send marked every alert on the order delivered) fixed and **proven in production**, R19 (SMS could not fire on any plan) found and fixed, R11 closed, R18 2-of-3. The critical path is now Partner-Dashboard and browser work: see [LAUNCH_PLAN.md](LAUNCH_PLAN.md) **§7**.
- **Tests**: 2,474 passing of 2,499, 25 skipped, 0 failing across 135 suites (2026-08-26). ⚠️ **Not certified stable** — [LAUNCH_PLAN.md](LAUNCH_PLAN.md) §6 R5 is open: the dominant mechanism is *wall-clock threshold assertions failing under machine load*, not order-dependent state leakage. Three full runs on 2026-08-26 gave 1 failure, 1 *different* failure, then 0 — each failing test green in isolation. **Never run `npm test` in two sessions at once.** Tightest offenders: `App.test.tsx:70` (<100 ms), `input-sanitization.test.ts:405` (<120 ms), `tests/performance/load.test.ts`, two thresholds inside `monitoring-service`, and `tests/integration/server-app.test.ts`. **Two lessons worth keeping.** (1) A test that passes only because a setup-file mock hides a real network call is not testing what its name claims. (2) **`__mocks__/pg.js` answers every `UPDATE` with `rowCount: 1`** — so no test using it can tell a one-row write from a four-row one. That is how R17 hid from 2,446 tests. For any assertion about what a statement *did*, use [`src/tests/helpers/pg-mem-schema.ts`](delayguard-app/src/tests/helpers/pg-mem-schema.ts), which runs the production `runMigrations()` against a real in-memory SQL engine. Local CI gate: `npm test && npm run lint && npm run type-check && npm run build` (run from `delayguard-app/`).
