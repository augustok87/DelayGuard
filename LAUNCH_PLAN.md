...
**Action `[HUMAN]`, highest priority:** Dev Dashboard → **DelayGuard → API access → Protected customer data** → request access, declaring the customer/order fields the app uses (name, email, phone, shipping address — see [DATA_AVAILABILITY_ANALYSIS.md](DATA_AVAILABILITY_ANALYSIS.md)) and the data-protection commitments. Approval is not instant, so **start it before any further code work**.

### R6 — Embedded HTML document sends no per-shop `frame-ancestors` `[AGENT]` — **submission-blocking**
...
*Rewritten 2026-07-30, after the app was installed on a real store for the first time. **R2 is no longer the frontier** — 3 of its 4 steps are proven and the 4th is blocked on a human approval, not on code. The next session's job is different: unblock R7, then close the two remaining agent-side blockers.*
...
| R2 step 2 (webhook subscriptions) | ⛔ blocked on **R7** — Shopify returns `[]` and refuses every topic |
| **R7 — Protected Customer Data approval** | ⛔ `[HUMAN]`, **top blocker**, has approval latency — must be requested first |
| R1 — SendGrid (credits + sender identity + template) | ⛔ `[HUMAN]`, unchanged, needs a paid-plan decision |
| R6 — per-shop `frame-ancestors` | ❌ open, `[AGENT]`, submission-blocking, not functional |
...
>
> **If R7 is still pending, do R6 instead** (per-shop `frame-ancestors` on the framed HTML document — approach described in §6 R6). It needs Koa to serve the app document, which also lets B2's install bootstrap do the HMAC check shopify.dev recommends. Verify with curl afterwards that `/` still returns the SPA shell for `embedded=1` and still 307s to `/auth` for an install request — those two curls are the only test that rule has.
>
> **Do not** re-audit Appendix A, re-verify Appendix B, or start C5 (expiring tokens, post-launch).
>
