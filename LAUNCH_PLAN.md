...
- [x] **H1. Pick pricing** — §5 D1. Recommended: **$7 Pro / $25 Enterprise + free tier** (matches code, most credible for a zero-review app; raiseable later — 8 public plans now allowed). *Taken as recommended; prod `GET /billing/plans` serves Free / $7 Pro / $25 Enterprise (probed 2026-07-28).*
- [x] **H2. Partner Dashboard:** confirm app exists as a **public app created before 2026-04-01** (determines token deadline = Jan 1, 2027); set App URL + OAuth redirect URL to the **stable production domain** (decide now, e.g. `https://delayguard-api.vercel.app` or a custom domain — record it as `SHOPIFY_APP_URL`). *Verified 2026-07-28: org Cautiva, client_id `e9d96cad…`, `application_url = https://delayguard-api.vercel.app`, redirect `/auth/callback`. App version `delayguard-1` dates to 2025-09-28 → predates 2026-04-01, so the token deadline is 2027-01-01 (C5 stays post-launch).*
...
- [x] **H5. Secrets into Vercel env:** `DATABASE_URL`, `REDIS_URL` (`rediss://` if Upstash), `SHOPIFY_API_KEY/SECRET`, `SHOPIFY_SCOPES` (incl. `read_customers`), `SHOPIFY_APP_URL`, `SHIPENGINE_API_KEY`, `SENDGRID_API_KEY`, `TWILIO_*`, `CRON_SECRET` (generate one). *Verified live 2026-07-28: prod `/health` reports Postgres + Redis healthy (79ms / 2ms), and all four `CRON_SECRET`-guarded `/api/cron/*` endpoints authenticate against the GitHub-Actions repo secret. **Outstanding:** `SENDGRID_DELAY_TEMPLATE_ID` (blocked on E1) — `email-service.ts` refuses to send delay email in production while it is unset.*
- [x] **H6. Run `shopify auth login`** once in a terminal so sessions can run `shopify app deploy` (webhook/toml push) non-interactively afterward. *Verified 2026-07-28 — `shopify app info` authenticates as augustok87@gmail.com; `shopify app deploy` has since run (C4).*
...
- [x] **C4 — `shopify.app.toml` + webhook registration `[AGENT+SECRET]`** (needs H6) (done 0e034a0b authored; `client_id` set 87f8aa4f; **deployed 2026-07-28** — `shopify app versions list` shows version `delayguard-3` ★ active. Design change from the original spec: `use_legacy_install_flow = true` (custom authorization-code OAuth) forbids app-specific webhook subscriptions in the toml, so only the three mandatory compliance topics are declared there; the functional topics `orders/updated` / `fulfillments/updated` / `orders/paid` register **per-shop after OAuth** via `webhook-registration-service.ts` (`265627c9`, `812b772b`)) · new `delayguard-app/shopify.app.toml` · Declare: `application_url = SHOPIFY_APP_URL`, redirect URLs, scopes (match C2), `[webhooks]` API version `2026-07`, subscriptions for `orders/updated`, `fulfillments/updated`, `orders/paid` → `/webhooks/...`, and the three compliance topics → the A3 GDPR URLs. Run `shopify app deploy`. **Accept:** deploy succeeds; Partner Dashboard shows the subscriptions; a test webhook from a dev store lands (verify via logs/DB row).
...
- [x] **D3 — Run migrations against prod `[AGENT+SECRET]`** (needs H5) (done f84839b2 — "all 8 tables migrated on prod Neon") · Run `npm run migrate:vercel` with prod `DATABASE_URL`; then add a documented release step so future deploys migrate deliberately (not per-request). **Accept:** prod psql shows all tables; the step is documented in `deploy.md`. *Verified functionally 2026-07-28 (no prod psql access from a session): all four `/api/cron/*` sweeps return `{"success":true, … errors:0}` in prod — those handlers query `orders`, `fulfillments`, `delay_alerts` and would error on a missing table.*
...
**Re-run 2026-07-28 20:22 UTC against `https://delayguard-api.vercel.app` — 9/9 pass.**

| Probe | Baseline (broken) | Required after deploy | Actual 2026-07-28 |
|---|---|---|---|
| `GET /` | 200 SPA shell, mock data | 200, embedded app loads real data | ✅ 200 |
| `GET /health` | (Koa unreachable; `api/health.ts` fakes healthy, `response_time: 0`) | 200 honest health from Koa | ✅ 200 `{"status":"healthy",…,"database":{"healthy",79ms},"redis":{"healthy",2ms}}` — real pings |
...
| `GET /api/cron/tracking-refresh` (no secret) | 404 | 401; with `CRON_SECRET` → 200 | ✅ 401; with secret → 200 `{"success":true,"stats":{…,"errors":0}}` (GH Actions run 30394657819) |
| `GET /billing/plans` | 404 | 200 plan info (single prefix!) | ✅ 200 — Free / $7 Pro / $25 Enterprise |
| `POST /webhooks/customers/redact` (no HMAC) | 404 | 401 | ✅ 401 |
| `GET /legal/privacy-policy` | 404 | 200 HTML | ✅ 200 (`/legal/terms-of-service` 200 too) |
| `GET /api` | 200 placeholder JSON ("Configure environment variables…") | gone or real | ✅ 404 — placeholder gone |
...
| 2026-07-28 | Deploy session | WS-A/B/C/D deploy-time | **The app went live.** Four defects that only deploying could surface (`f84839b2`, `23c73260`): the `api/[[...path]].ts` bracket catch-all doesn't resolve outside Next.js (every route 404'd) → renamed to `api/index.ts`; `PORT`/`HOST` were `required` and `process.exit(1)`-ed on serverless cold start; `parseRedisUrl` dropped the `rediss://` TLS scheme so Upstash closed the connection; husky's `prepare` exited 128 on Vercel's git-less build. **D3** prod migration run (8 tables on Neon). **C4** `shopify app deploy` → version `delayguard-3` active; functional webhooks moved to per-shop post-OAuth registration (`265627c9`, `812b772b`) because `use_legacy_install_flow = true` forbids app-specific toml subscriptions. Vercel Hobby caps crons at once/day, so sweeps moved to a GitHub Actions scheduler (`b70e969d`). |
| 2026-07-28 | Verification + CI-green session | Probe matrix, H-gate audit, CI | Re-ran the **Appendix B probe matrix against prod — 9/9 pass** (recorded above). Audited the human gate against live evidence: **H1, H2, H5, H6 confirmed done**; H3/H4/H7 need the human (Partner Dashboard / ShipStation account). **D3 verified functionally** via the four prod cron sweeps returning `errors:0`. Fixed the two workflows that had been red on `main` — all failures were test-environment defects, not product bugs: (a) `tests/unit/services/monitoring-service.test.ts` never mocked `fetch`, so `checkExternalAPIs()` made **live** HEAD requests to ShipEngine/SendGrid/Twilio and failed on CI runners; (b) two `*-schema.test.ts` files hardcoded the passwordless local dev `DATABASE_URL`, killing the CI Postgres job with `SASL: client password must be a string`; (c) two wall-clock assertions flaked on shared runners (`input-sanitization.test.ts` 120ms budget, `optimized-database.test.ts` `toBeGreaterThan(3000)` receiving exactly 3000). Also added the missing `AbortController` timeout to `checkExternalAPIs()` per the CLAUDE.md third-party invariant. A fifth CI-only failure resisted local reproduction, so the assertion was instrumented to print a per-check breakdown — the next CI run named it: **`checkApplication()` computed `heapUsed / os.totalmem()`**, the V8 heap against total system RAM (1.1% on a 34 GB dev box, so the 80/90 thresholds could never fire; 93% on a CI runner, falsely reporting the app unhealthy). Replaced with headroom against the V8 heap ceiling (`heapUsed / v8.getHeapStatistics().heap_size_limit`), with tests pinning 50/85/95% → healthy/degraded/unhealthy. **Blocker remaining:** E1 needs the real `SENDGRID_API_KEY` (local `.env` holds a placeholder; SendGrid returns 403), plus a dev store for the end-to-end `/api/test-alert` run, plus H3/H4/H7/H8/H9. |
