...
**Still open (deliberately not fixed):** the per-keystroke write itself. `NotificationPreferences` calls `saveSettings` on every `onChange` with no debounce, so a 20-character email is ~20 `PUT /api/settings` calls and ~20 `data_access_log` rows. `SettingsCard` debounces at 1 s and should be the model. Symptom fixed; waste not.
