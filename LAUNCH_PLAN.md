...
*Written 2026-07-29 at the end of the truth-pass session. The next session is a **live-verification** session, not a coding session: the app is deployed and the code is believed correct, so the job is to prove it against a real merchant and fix what reality breaks.*

**Paste this as the opening prompt:**

> You are executing DelayGuard's launch plan. Read `LAUNCH_PLAN.md` **§6 (Remaining blockers)** and **§7** first — the app is already deployed and live at `https://delayguard-api.vercel.app`, so §§1–4 are history and Appendix A is verified ground truth you must not re-audit. Follow `CLAUDE.md` and the matching `.claude/rules/*.md` for any area you touch: TDD (failing test first), and the local CI gate (`npm test && npm run lint && npm run type-check && npm run build` from `delayguard-app/`) before you claim anything is done. Never `--no-verify`.
>
> **Your objective: close §6 R2 — get one real merchant install working end to end, and fix whatever it exposes.** Nothing in this app's core path has ever run against a live Shopify store. In dependency order, prove each step and fix what breaks: (1) OAuth authorize → callback → `state` verification → token exchange → `upsertShop`; (2) per-shop webhook registration via `webhook-registration-service` — **this is best-effort and only logs on failure, so verify it actually created the three subscriptions rather than trusting the install redirect**; (3) a real `orders/updated` payload passing HMAC and landing in Postgres; (4) the embedded dashboard loading inside Shopify admin with a working session token and rendering real (probably empty-state) data.
>
> I will drive the browser for the dev-store install; you drive the diagnosis — tell me exactly what to click, then verify each step yourself against Vercel logs, the production DB, and the Shopify Admin API rather than taking my word for it. Treat a silent success as suspicious: confirm the webhook subscriptions exist by querying Shopify, and confirm rows landed by querying Postgres.
>
> **Secondary objective, only if I confirm the SendGrid plan is upgraded:** finish §6 R1/E1 — domain-authenticate `delayguard.app` (I'll add the DNS records you specify), then create the dynamic template with a temporary Full Access key I'll provide, hand me the `d-…` ID for Vercel, and verify with `/api/test-alert`. Remind me to delete the temporary key afterwards.
>
> Document as you go: update `LAUNCH_PLAN.md` (§6 blocker status, Session Log, and rewrite §7 for the session after this one), `PROJECT_OVERVIEW.md`, and `CHANGELOG.md` in the same commits as the work. If you find a new blocker, add it to §6 with the evidence that proves it — the way R1 was proven with live API calls and DNS lookups, not assumed.

**Why this and not something else:** R2 is the only remaining item that can still surface *unknown* bugs — everything else on the list is a known quantity (a payment, a form, a dashboard click). It also gates H8's screencast, which needs a working app to film. R1 is more severe but is currently blocked on a human purchase decision, so it rides along as a secondary objective.
