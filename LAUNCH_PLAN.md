...
| 2026-09-21 | **R27: contact details would not save on a fresh install** | Notification Preferences → `PUT /api/merchant-settings` | Filming the screencast (H8) on a **new** dev store (`delayguard-demo`, created so the install could be filmed without uninstalling from `delayguard-dev`). Install and OAuth grant worked first time. Entering a merchant email with the optional phone empty toasted `Invalid phone format - must contain at least 10 digits` and saved nothing, because the form sends every contact field and the untouched phone arrived as `""`. A reviewer would hit it on the first settings screen. Fixed TDD (v1.79): blank email or phone now means "not provided". **Verifying that fix live exposed a second defect:** the save returned 200 but the form reloaded empty, because the client never called `GET /api/merchant-settings`; `fetchSettings` now reads it. Also seen, not fixed: clearing the threshold field to retype it autosaves the transient `0` and toasts a validation error. |
