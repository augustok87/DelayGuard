...
### R2 — No end-to-end verification on a real dev store `[HUMAN]` + `[AGENT]` — **in progress**
...
- **B1 — the OAuth authorize URL carried a corrupted scope. ✅ Fixed in code; needs the Vercel env value fixed too.** Live `GET /auth?shop=…` 302'd to Shopify with `scope=…%2Cread_products%2Cread_customers%0A`. The `%0A` is a newline: `SHOPIFY_SCOPES` was saved in the Vercel dashboard with a trailing line break, and `app-config.ts` split it on `,` without trimming, so the final scope was literally `read_customers\n`. Shopify either rejects the grant outright or drops `read_customers` — the scope Phase 2.1.a customer intelligence depends on. `parseScopes()` now trims every entry, drops empties, and falls back to the code defaults when nothing usable remains (5 new tests). **The env value must still be re-saved without the newline** — the code fix stops a corrupt value from reaching Shopify, but a whitespace-only-difference env var should not be left wrong in production.
...
### R6 — Embedded HTML document sends no per-shop `frame-ancestors` `[AGENT]` — **new, submission-blocking**
...
*Rewritten 2026-07-29 (b), mid-R2. The previous kickoff sent a session at "prove the install works"; it got two commits in and found the install could never have worked. **R2 is still the objective** — the code-side blockers to the first install are now fixed (`560b86b0`) but the live walk itself has not happened.*

**State at handoff — read this before writing any code:**

| | Status |
|---|---|
| B1 trailing-newline scope | ✅ fixed in code. **Confirm the Vercel `SHOPIFY_SCOPES` value was re-saved without the newline** — if not, do that first (it is a dashboard edit, not a code change). |
| B2 app URL → OAuth bootstrap | ✅ fixed in `vercel.json`. **Unverified against the deployment** — the very first thing to do is curl it (below). |
| The live install walk (steps 1–4) | ❌ not started. |
| R6 per-shop `frame-ancestors` | ❌ open, submission-blocking, fix sequenced after the walk. |

**Paste this as the opening prompt:**

> You are executing DelayGuard's launch plan. Read `LAUNCH_PLAN.md` **§6** and **§7** first — the app is deployed and live at `https://delayguard-api.vercel.app`, so §§1–4 are history and Appendix A is verified ground truth you must not re-audit. Follow `CLAUDE.md` and the matching `.claude/rules/*.md`: TDD (failing test first), and the local CI gate (`npm test && npm run lint && npm run type-check && npm run build` from `delayguard-app/`) before claiming anything is done. Never `--no-verify`.
>
> **Objective: finish §6 R2 — one real merchant install, proven end to end.** Two install blockers were fixed last session but the walk never ran. Start by proving the fixes actually landed in production, in this order:
>
> 1. `curl -sI "https://delayguard-api.vercel.app/?shop=<store>.myshopify.com&hmac=x&timestamp=1"` → expect **307 to `/auth`**. If it returns the SPA (200 HTML) the `redirects` rule in `vercel.json` is not matching — that rule has no unit test by design, so this curl *is* its test.
> 2. `curl -si "https://delayguard-api.vercel.app/auth?shop=<store>.myshopify.com" | grep -i location` → the `scope=` param must contain **no `%0A`, `%0D` or `%20`**.
>
> Then walk the install and prove each step against the source of truth, not the UI: (1) OAuth authorize → callback → `state` → token exchange → `upsertShop` (verify: a `shops` row with a non-empty `scope` array, and an `app_settings` row); (2) per-shop webhook registration — `webhook-registration-service` is **best-effort and only logs on failure**, so query Shopify's Admin API for the three `webhookSubscriptions` rather than trusting the install redirect; (3) a real `orders/updated` payload passing HMAC and landing in `orders` (a `delay_alerts` row only appears if a tracked fulfillment is actually late — don't treat its absence as failure); (4) the embedded dashboard loading in Shopify admin with a working session token, rendering real (probably empty-state) data.
>
> The human drives the browser; you drive the diagnosis — say exactly what to click, then verify it yourself against Vercel logs, the production DB, and the Shopify Admin API. **Treat a silent success as suspicious.**
>
> After the walk lands, fix **§6 R6** (per-shop `frame-ancestors` on the framed HTML document) — the approach is described in R6 and needs Koa to serve the app document, so do it only once the install is proven and you can re-verify it end to end.
>
> **Secondary, only if the human confirms the SendGrid plan is upgraded:** finish §6 R1/E1 — domain-authenticate `delayguard.app` (they add the DNS records you specify), create the dynamic template with a temporary Full Access key, hand back the `d-…` ID for Vercel, verify with `/api/test-alert`, and **remind them to delete the temporary key**.
>
> Document as you go: `LAUNCH_PLAN.md` (§6 status, Session Log, and rewrite §7 for the session after you), `PROJECT_OVERVIEW.md`, `CHANGELOG.md` — same commits as the work. New blockers go in §6 **with the evidence that proves them**, the way R1 was proven with live API calls and DNS lookups.

**Why this and not something else:** unchanged, and reinforced by what last session found. R2 is the only remaining item that can still surface *unknown* bugs — and it surfaced two immediately, both of which would have made a merchant's very first click fail silently. Everything else on §6 is a known quantity (a payment, a form, a dashboard click). R2 also gates H8's screencast.

**The lesson worth carrying:** both defects sat behind a 2,385-test green suite and a fully green Appendix B probe matrix. Neither artifact was wrong — they just never exercised the merchant's entry point. When a path has never run in production, **walk it**; don't infer it from the tests that cover its parts.

**What the session should NOT do:** re-audit Appendix A; re-verify the Appendix B probe matrix (green as of 2026-07-29); rebuild anything in Appendix A.13's "genuinely real" list; or start C5 (expiring tokens — post-launch, deadline 2027-01-01).
