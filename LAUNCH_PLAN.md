...
### R24 — Two of the three delay rules have never worked, and cannot on the current ShipEngine plan `[HUMAN]` — **new 2026-08-26, this is H7 and it is now PROVEN, not "unverified"**
...
1. **§6 R24 / H7 — the listing's headline capability does not work.** It sells *"real-time monitoring across major carriers"* and *"monitors carrier tracking data"*, while ShipEngine refuses `/v1/tracking` on the current plan. **Submit only after H7 is purchased and the probe returns 200.**
...
| **R24 / H7 — carrier tracking is refused by the plan** | ⛔ `[HUMAN]`, **now PROVEN and submission-blocking.** `/v1/tracking` returns *"You must upgrade your billing plan"*, so **RULES 2 and 3 have never fired** — production holds 4 `WAREHOUSE_DELAY` alerts and **0** `tracking_events`. The listing sells carrier tracking as the headline feature, so unlike R20 this cannot be fixed by editing copy |
