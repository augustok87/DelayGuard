...
*Rewritten 2026-08-26. **The three agent-side blockers on the critical path are closed, deployed and proven against production.** What remains between here and submission is almost entirely Partner-Dashboard and browser work. The goal is still **submission**.*
...
| `[HUMAN]` — browser, in this order | `[AGENT]` — repo |
|---|---|
| 1. **R8** — Cloudflare Email Routing (+ the `include:sendgrid.net` SPF edit), then *receive* a test message | 1. Verify R8's DNS by `dig` once it's live, and re-confirm the SendGrid DKIM chains still resolve |
| 2. **H3** — App Pricing plans (Free / $7 Pro / $25 Enterprise) | 2. After H3: re-check that `getCurrentPlan` returns a paid tier and that SMS actually dispatches (R19's fix is untested against a real paid plan) |
...
> You are executing DelayGuard's launch plan, and the goal is **App Store submission**. Read `LAUNCH_PLAN.md` **§7** then **§6** — the app is live at `https://delayguard-api.vercel.app`, installed on `delayguard-dev.myshopify.com`, and as of 2026-08-26 the notification pipeline is **correct per alert and proven so in production**. §§1–4 are history. **Appendix A is ground truth: do not re-audit it.** Do not re-verify Appendix B, do not re-verify R1/R2/R6/R7/R17/R19, do not start C5.
>
> **Division of labor: I drive the browser and any purchase; you drive the diagnosis.** Tell me exactly what to click, then verify each step yourself against Vercel logs, the production DB, and the Shopify/SendGrid APIs rather than taking my word for it. **Treat a silent success as suspicious.**
>
> **The remaining work is mostly mine, not yours.** R8 (Cloudflare Email Routing — note §6's corrected SPF guidance), H3 (App Pricing plans, blocks all revenue), one real test order on the dev store, H7, then H8 → H-4 → H9. Tell me early what to start so it runs in parallel.
>
> **Your side:** (a) once R8 is live, verify the MX/SPF by `dig` and confirm SendGrid's DKIM chains still resolve; (b) once H3 exists, prove SMS actually dispatches — R19 fixed a bug that made it impossible on every plan, and that fix has never been exercised against a real paid subscription; (c) once a real order exists, prove the webhook landed in Postgres and **read the resulting email**; (d) close R18 defect 3 when I mint a Full-Access SendGrid key — `npm run sendgrid:create-template`, new `d-…` id into `SENDGRID_DELAY_TEMPLATE_ID`, redeploy. **`create-sendgrid-template.ts` is currently ahead of the deployed template.**
>
> **Follow `CLAUDE.md` and the matching `.claude/rules/*.md`. TDD is not optional:** write the test, run it against the *broken* code, watch it fail, then fix. A test that passes in both states gets deleted, not kept. Local CI gate before any commit. **Never `--no-verify`.** ⚠️ **Never run `npm test` in two sessions at once.**
>
> **Operational facts (do not rediscover):** deploys are **not** git-triggered — `npx vercel --prod --yes` from `delayguard-app/`. Migrations: `npm run migrate:vercel`. **`vercel env pull` masks every `Sensitive` value** — verify env through the running deployment. `shopify app env show` 403s (§6 R9), so any `/api/*` check is a browser action. `DATABASE_URL` *is* readable — production Postgres is your best evidence source. The production **SendGrid key is `mail.send` only**; `/v3/templates` 403s. Sending domain is **`delayguardapp.com`**; `delayguard.app` is **not ours**.
>
> **There is now a real-SQL test harness** (`src/tests/helpers/pg-mem-schema.ts`): it swaps `pg` for pg-mem and builds the schema by running the production `runMigrations()`. Use it for anything asserting what a statement *did* rather than that it was issued — `__mocks__/pg.js` answers every `UPDATE` with `rowCount: 1`, which is how R17 hid from 2,446 tests.
>
> **Document as you go** — `LAUNCH_PLAN.md` (§6 status, Session Log, and rewrite §7), `PROJECT_OVERVIEW.md`, `CHANGELOG.md` — in the same commits as the work. New blockers go in §6 with the evidence that proves them.
>
> **Carried lesson: when you form a belief about the world outside the repo, write down what would falsify it and go observe that.** "Consistent with" is not "proven." An authorization error names a relationship, not a state. A check that has never failed is not a check. **And a single passing run is not evidence of determinism** — that one cost a wrong causal claim mid-session today.

**Why this order.** The agent column is short because the merchant-harming bugs are gone: R17 is proven in the real pipeline and R19 removed a dead paid entitlement before H3 could make it visible. What is left is gated on things only a human can do — DNS, a pricing plan, a real purchase, a SendGrid key, and Shopify's own review queue.

**The lesson this session added.** R19 was not on anyone's list. It surfaced because the R17 harness ran the *real* SELECT against the *real* schema and printed `shopDomain: undefined` — a value every existing test supplied by hand in its fixture. **A mock that returns the row you wish the query returned cannot tell you the query is wrong**, and a driver stub that always reports `rowCount: 1` cannot tell you a statement touched four rows. Both of this session's database defects were invisible to unit tests *by construction*, and both fell out of one real schema. The 2026-08-25 lesson was "read the artifact the system produces"; today's is its sibling — **run the real statement against the real shape.**
