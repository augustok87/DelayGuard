...
### ~~R8 — No working support mailbox, and the listing claims one~~ `[HUMAN]` — ✅ **CLOSED 2026-09-16**

**`support@delayguardapp.com` receives mail, proven by receipt — not by configuration.**

Cloudflare Email Routing was enabled on `delayguardapp.com`: destination `augustok87@gmail.com` created and verified, rule `support@` → forward created and enabled, both over the API; the MX records were written by the dashboard's *Verify and activate* step, which the API token lacked permission to reach. Verified with `dig @1.1.1.1` **after** activation:

| Record | Value | |
|---|---|---|
| MX ×3 | `route1/2/3.mx.cloudflare.net` (priority 38 / 80 / 9) | ✅ live |
| SPF | `v=spf1 include:_spf.mx.cloudflare.net include:sendgrid.net ~all` | ✅ **both** includes |
| `s1`/`s2._domainkey` | resolve into `u56367697.wl198.sendgrid.net` | ✅ sending path unharmed |
| `em2368` / `url7655` | resolve into `sendgrid.net` | ✅ |
| DMARC | `v=DMARC1; p=none;` | ✅ unchanged |

**Accept criterion met** (unchanged since this blocker opened): a message sent **from an address other than the destination** arrived in the destination **Inbox, not Spam**.

**⚠️ This plan's SPF warning was wrong twice, in opposite directions — and the second time it looked vindicated.** §7 first said activation *rewrites* the zone's SPF. §6 corrected that to *adds* one, because `dig` showed the zone had no apex TXT at all. By activation time an SPF record did exist (written during API setup), and the wizard's own preview listed `v=spf1 include:_spf.mx.cloudflare.net ~all` among the records it would write — dropping SendGrid, exactly the original warning. **It merged instead of replacing.** The durable lesson is not about SPF: the same vendor behaviour was predicted three times and settled every time by the `dig` afterwards. Predict, then go observe — the observation is the artifact worth keeping.

**A self-addressed test is not a test.** The first attempt sent from `augustok87@gmail.com` to `support@` produced nothing in the inbox: Gmail dedupes a message addressed to the account that sent it. Cloudflare noticed and emailed an explanation — and *that notice* was the first real evidence the rule had matched, since it could only exist if the message had been received and routed. Read naively, a silent inbox would have said "routing is broken." **The absence of a delivered message was not the absence of delivery.**

**⛔ Still open, and it is not `support@`:** the listing also advertises **`sales@delayguardapp.com`**, and **no routing rule exists for it** — only `support@` was ever created. Either add the second custom address in Email Routing or delete the line from the listing. Shipping a listing that names an address which discards mail is the precise defect this blocker was opened for, and closing R8 on `support@` alone would recreate it.

---

*Original problem statement and the two superseded SPF warnings, kept because the correction history is the point:*
...
### R24 — Two of the three delay rules have never worked `[AGENT — RESOLVED without any vendor (v1.76)]`

**Status 2026-09-16: delay detection no longer depends on a carrier API at all.** EasyPost has been in anti-fraud manual review since signup on 2026-08-26 — three weeks, no key, no committed date — so waiting stopped being a strategy. Shopify already pushes `shipment_status` on every `fulfillments/updated` webhook; the field was declared on `FulfillmentWebhookPayload` and silently discarded by `upsertFulfillment`. It now drives `orders.tracking_status` and `orders.last_tracking_update`, which is exactly what **RULE 3 (STUCK_IN_TRANSIT)** measures. No vendor, no account, no key, no cost.

**Two landmines the fix uncovered, both of which would have shipped:**
1. `EASYPOST_API_KEY` was in `requiredVars`, and production holds `SHIPENGINE_API_KEY`. Deploying v1.75 would have **failed at cold start** — trading a degraded feature for a dead app. The key now warns instead of gating boot.
2. `CarrierService`'s constructor throws without a key and `TrackingIngestService` built one **eagerly**, so the fulfillment webhook would have thrown *before* reaching the Shopify write — 500ing the webhook in precisely the case the fallback exists for. Now built lazily. Pinned by a test that deliberately leaves `CarrierService` **unmocked**, since a mocked constructor cannot throw and would have passed against the broken code.

**The honest limit, pinned by a test.** Shopify's value set has no `delayed` member, so this source never yields `DELAYED`. RULE 2's `DELAYED_STATUS` branch and the ETA-based `DATE_DELAY` rule stay dark until a carrier key exists. Lateness is caught by RULE 3's time-based check, and `attempted_delivery` / `failure` raise `EXCEPTION`.

**Listing implication.** The headline claim must not promise carrier ETAs while only RULE 1 and RULE 3 run. Soften the copy before H9, or wait for a carrier key.

<details><summary>Superseded: the EasyPost migration (v1.75) and the original ShipEngine refusal</summary>

EasyPost remains fully implemented and tested; it activates the moment a key exists, and the Shopify path then becomes a safety net rather than the only source. ShipEngine refuses `/v1/tracking` on the Free plan — proven on UPS *and* USPS, both connected, so it is a plan entitlement rather than a carrier gap. ShipStation Advanced is $75/mo against a $7/mo product (~11 merchants to break even, at zero), which is why the answer was migration rather than purchase (decision **D4**).
</details>
...
### R28 — Two overlapping dispatches sent the same customer the same email twice `[AGENT]` — **new 2026-09-22, fixed same session (v1.81)**

**Reproduced, not reasoned about.** `src/tests/integration/notification-double-send-race.test.ts` runs the production `runMigrations()` against pg-mem and asserts how many sends left the building. Against the shipped code: `expect(sendDelayEmail).toHaveBeenCalledTimes(1)` → **Received number of calls: 2**.

**Why it survived 2,570 tests.** `queue/sweeps/notification-sweep.ts` stated in its header that the processor's own re-check made overlapping ticks safe. It did not. The processor read `email_sent`, awaited the provider, and wrote the flag afterwards — so the whole provider call was a window in which a second dispatch read `FALSE` and sent again. The flag recorded what finished; nothing claimed what started. A false comment is why nobody added the guard.

**Why the existing doubles could not see it** — the same defect class as R17. `__mocks__/pg.js` answered every `UPDATE` with `rowCount: 1` but `rows: []`; `wireQuery` in `tests/unit/queue/notification.test.ts` answered `UPDATE delay_alerts` with `[]`. A `RETURNING` claim therefore read as lost on every call. Fifteen dispatch tests broke on the harness, not the code, which measures exactly how blind those 32 tests were.

**Trigger in production.** `/api/cron/notification-dispatch` has no lease, no advisory lock, and answers GET and POST behind a bearer secret. The Actions schedule, its `curl --retry 2`, a `workflow_dispatch` run and a manual curl each put two sweeps in flight.

**Fix (v1.81).** `dispatchOnce` claims one channel of one alert — `UPDATE … SET <flag> = TRUE WHERE id = $1 AND <flag> = FALSE RETURNING id` — before calling the provider, and skips the send on an empty result. A failed send releases the claim. `notification_sent_at` is stamped only after success.
...
Session tokens passing proves Shopify **has** session data for this app, so the App Bridge failure is a real differential, not a stale or empty check.

**The served document is correct** — `<meta name="shopify-api-key">` then `<script src="https://cdn.shopify.com/shopifycloud/app-bridge.js">` as the first script in `<head>`. The problem is what the bundle does afterwards.

**Measured cause.** `src/components/AppProvider.tsx` — the root provider rendered by `index.tsx` — wraps the entire app in `ShopifyProvider`, which calls `createApp()` from the **legacy npm `@shopify/app-bridge` v3.7.10**. So the app loads the CDN App Bridge *and* boots the previous-generation bridge on top of it:

- The deployed `vendors.097e43e6c41fc45975cf.js` contains **119 `APP::` legacy action types**, including `APP::APP_ERROR::MISSING_APP_BRIDGE_MIDDLEWARE`.
- Inside the real admin iframe the app's own frame logs **`✅ App Bridge initialized successfully`** — that is `createApp` succeeding, measured in the embedded context.
- The first embedded load also produced `Failed to execute 'postMessage' on 'DOMWindow': The target origin provided ('https://delayguard-api.vercel.app') does not match the recipient window's origin ('https://admin.shopify.com')` — the legacy bridge messaging the wrong origin.
...
The live listing's `websiteUrl` is `https://delayguard-api.vercel.app`, which serves the **embedded app document**. Loaded outside the Shopify iframe — exactly what a reviewer clicking the listing does — it renders the dashboard with a red banner **"Error: Missing Authorization header"**, `0 / 0 / 0` counters and **"Not Connected"**. Screenshotted 2026-09-22. It needs a real landing page, or the field needs to point somewhere that is not the app shell.

### ~~R31 — Protected customer data: the Phone field is justified by SMS, which cannot be sold or sent~~ `[AGENT]` — ✅ **FIXED IN THE DASHBOARD 2026-09-22**

**Phone is now deselected entirely.** Its reason had read *"Send SMS delay alerts to buyers when a merchant enables SMS on a paid plan"* — there has been no paid plan since v1.80 and the Twilio trial cannot send at all. Phone is only ever read for SMS, so removing it is honest rather than a reword. Verified after saving: the Phone row carries no Reason line, and the words "SMS" and "paid plan" appear nowhere on the page.

**Address reworded.** It claimed *"Compute the delay and revised ETA from the shipping destination"*, which nothing performs. It now reads *"Show the merchant the order's shipping destination on the delay alert"* — what `api-mappers.ts` actually does with those columns. Verified after save.

⚠️ Watch for knock-on: with Phone deselected Shopify will stop sending customer phone numbers, so `orders.customer_phone` will be null on new orders. Every send path already requires it to be non-null, so SMS simply never selects — no code change needed, and none was made.

Read from the Protected customer data page 2026-09-22, verbatim:

> **Phone** — Reason: Customer service, Other (*"Send SMS delay alerts to buyers when a merchant enables SMS on a paid plan."*)

There is **no paid plan** (Pro and Enterprise were deleted in v1.80) and **SMS cannot send at all** (R20: Twilio trial, no number, 0 messages ever). The listing has not mentioned SMS since v1.78. Shopify states on that same page that it reviews whether *"Data use matches the functionality described in your App Store Listing"* — so the stated reason for Phone matches nothing in the listing and nothing the app can do. **Deselect Phone**, or reword it; deselecting is the honest option because phone is only ever read for SMS.
...
### ~~R30 — The listing's own website URL shows the app erroring~~ `[AGENT]` — ✅ **FIXED + DEPLOYED 2026-09-22 (v1.84)**

Fixed in code rather than by editing the listing: `/` now answers by audience. A request Shopify framed (`embedded`, `id_token`, `host` or `shop`) gets the app document unchanged; a bare request gets a self-contained informational page — no bundle, no external stylesheet, no web font, so the app's own CSP has nothing to block. `websiteUrl` was always the right field; the URL just needed to mean something. **Verified live**: the bare URL returns the landing page with the legal links and support address and no `id="root"`; the embedded URL still returns the app with `app-bridge.js`.
...
### ~~R37 — Install asked for two write scopes the app never uses~~ `[AGENT]` — ✅ **CLOSED 2026-09-22** (code v1.83, released as app version `delayguard-2-4`)
...
**Released 2026-09-22.** `shopify app deploy` initially failed with *"Could not find a Shopify app configuration file"* — it was run from the repo root, and `shopify.app.toml` lives in `delayguard-app/`; the CLI searches the working directory and its **parents**, never its children. Re-run from `delayguard-app/`, the confirmation listed `access_scopes` as the only `(updated)` field, and released as **`delayguard-2-4`**.

The second half of this — the Vercel env var — turned out to still be live and is tracked as **R39** below.

### ~~R39 — A stale env var kept production asking for write scopes, and the guard could not see it~~ `[AGENT]` — ✅ **FIXED 2026-09-22 (v1.85)**

**`minimum-scopes.test.ts` is titled "OAuth scopes are the minimum the app actually uses" and it passed — while production asked for two write scopes.** Read live off the production authorize redirect on 2026-09-22:

```
GET /auth?shop=delayguard-dev.myshopify.com
→ scope=read_orders,write_orders,read_fulfillments,write_fulfillments,read_products,read_customers
```

`SHOPIFY_SCOPES` was set in Vercel **338 days** before the scopes were reduced, and `parseScopes` returned whatever it said (`app-config.ts:80`). The test only ever read `DEFAULT_SHOPIFY_SCOPES`, a constant the env overrides — so it asserted the honest value while the dishonest one shipped. That is the R21 shape again: a check that cannot fail on the thing it claims.

Deleting the env var was not available (it is on `environment.ts`'s required-vars list, and the agent's tooling refuses secret-store writes), and more to the point an env var that can **widen** the app's access past what its code can justify is the wrong mechanism. `parseScopes` now intersects the env value with `DEFAULT_SHOPIFY_SCOPES`: the env may narrow and never widen, and adding a scope is a code change — the same place the review evidence for needing it lives.

RED first: three new assertions failed against the old parser, the first replaying production's exact scope string. The narrowing case passed throughout and is kept as a documented regression guard. The three pre-existing whitespace tests used `write_orders` as filler and were re-fixtured onto declared scopes; they still fail if trimming is removed, because an untrimmed `read_customers\n` no longer matches a declared scope.

**Verified live after deploy** by re-reading the same authorize redirect.
...
| **R8 — support mailbox** | ✅ **CLOSED 2026-09-16.** MX live, SPF carries **both** `_spf.mx.cloudflare.net` and `sendgrid.net`, SendGrid's DKIM chains intact — all `dig`-verified after activation. A message from an outside sender **arrived in the Inbox**. ⚠️ `sales@` has no rule yet; add it or drop it from the listing |
| **H3 — App Pricing plans** | ✅ **DONE 2026-09-20.** Shopify App Pricing was already enabled; three public plans now exist — `free` (Free), `pro` ($7/mo, 14-day trial), `enterprise` ($25/mo, 14-day trial) — verified on the plans list, not the forms. Names match `billing-service.ts`'s substring match. Name and handle are permanent in Shopify |
...
| **R24 / H7 — carrier tracking** | ✅ **RESOLVED (v1.76)** without any vendor. Shopify's own `shipment_status` now drives `orders.tracking_status` / `last_tracking_update`, so **RULE 3 fires with no carrier API, no account and no key**. EasyPost stays implemented and activates on a key. ⚠️ Remaining: `DELAYED` and `DATE_DELAY` need a carrier key, so **soften the listing copy before H9** |
...
|---|---|---|
| 1 | ~~**R8** — Cloudflare Email Routing~~ ✅ **done 2026-09-16.** ⚠️ `sales@` still unrouted | ✅ `dig` confirms MX, **both** SPF includes and both `_domainkey` chains; the SPF was merged, not replaced. Receipt proven by an external-sender test |
| 2 | ~~**H3** — App Pricing plans~~ ✅ **done 2026-09-20** | Confirm `getCurrentPlan` returns a paid tier once a real merchant subscribes — still owed |
...
| 5 | ~~**H7** — confirm ShipStation Advanced~~ **obsolete** — R24 closed with no carrier vendor | — |
| 6 | **H8 screencast — the only listing field left.** 3–8 min on YouTube or Loom, install flow + core features. Then **H-4** AI self-review → **H9** submit | Listing verified at **1 issue** after reload 2026-09-21: every other field filled, 3 screenshots + feature image uploaded at 1600×900, SMS mentions 0 |
...
| 2026-09-16 | **R24 resolved with no vendor at all** — Shopify's `shipment_status` becomes the delay source (v1.76) | webhooks + TrackingIngestService + boot validator | **Three weeks in EasyPost's anti-fraud queue made waiting the wrong plan.** The data was already arriving: `FulfillmentWebhookPayload` has declared `shipment_status` since the webhook was registered, and `upsertFulfillment` threw it away every time. Shopify polls its own supported carriers, so this is carrier tracking with no vendor, no key and no cost — and it writes the two columns RULE 3 actually measures. **The fix uncovered two landmines that would both have shipped.** `EASYPOST_API_KEY` was in `requiredVars` while production holds `SHIPENGINE_API_KEY`, so **v1.75 would have failed at cold start** — a degraded feature traded for a dead app. And `CarrierService` throws without a key while `TrackingIngestService` built one **eagerly**, so the webhook would have thrown *before* the Shopify write, 500ing the exact path the fallback exists for; that one was caught only by deliberately leaving `CarrierService` **unmocked**, because a mocked constructor cannot throw and would have passed against the broken code. **The capability limit is pinned, not glossed:** Shopify has no `delayed` status, so `DELAYED` and `DATE_DELAY` stay dark until a carrier key exists, and a test asserts no Shopify value ever maps to `DELAYED`. Also audited the project's paid surface at the user's request: Twilio is a **Trial with 0 numbers and 0 messages**, ShipEngine Free, EasyPost $0 base, and Sentry/Supabase/Railway/LogRocket/Datadog/Stripe appear only in planning docs — never in code or env. The one real recurring charge found was **`delayguardapp.com`**, registered at Cloudflare 2026-08-25, renewing 2027-08-25, which is load-bearing as the SendGrid sending domain. Gate 2,554 passing / 2,579, 0 failing, 141 suites. |
| 2026-09-16 (b) | **Self-inflicted `/monitoring/health` 503, caught by probing the running thing** | monitoring route | The v1.76 deploy landed correctly — the live check is named **EasyPost**, which is the new code — but `/monitoring/health` went **503** where it had been 200 an hour earlier. Body: `"status": "degraded"` with **one** degraded check (the deliberately unconfigured carrier) while Database, Redis, SendGrid, Twilio and the application were all healthy. Cause: the route mapped `checks.every(healthy) ? 200 : 503`, harmless only while nothing was ever degraded. Since an unconfigured carrier is now the **steady state**, the endpoint would have stayed permanently red — the same defect as a check that cannot fail (R21), and it would have trained us to ignore the one endpoint that reports real outages. **Found only because the deploy was followed by a probe rather than by reading the CLI's success message** — the same habit that caught the 4-minute outage. Pre-deploy I had checked `/health`'s mapping (degraded → 200) and wrongly generalised it; `/monitoring/health` is a **different route** with its own mapping. Fixed in v1.77: unhealthy → 503, everything else → 200, body still distinguishing healthy from degraded. Four route tests pin both halves; three of the four failed against the broken mapping first. Production re-probed: `/health` 200, `/monitoring/health` **200**, both legal pages 200. |
| 2026-09-16 (c) | **R8 closed — `support@delayguardapp.com` actually receives mail** | DNS + Cloudflare Email Routing | **The mailbox is proven by a received message, not by a green dashboard.** Destination and forwarding rule were created over the Cloudflare API; the MX records needed the dashboard's *Verify and activate*, which the token could not reach. `dig @1.1.1.1` afterwards: MX ×3 live, SPF `v=spf1 include:_spf.mx.cloudflare.net include:sendgrid.net ~all`, both SendGrid `_domainkey` chains and both branding CNAMEs intact, DMARC unchanged. **Two things worth carrying.** (1) *A prediction about a vendor was wrong twice in opposite directions* — this plan said activation would rewrite the SPF, then corrected itself to say it would add one, then the wizard's preview showed a replacement that dropped `sendgrid.net`, and it **merged** instead. Three predictions, one reliable arbiter: the `dig` afterwards. (2) *A silent inbox was not a failure.* The first test sent from the destination account to itself and Gmail deduped it; the only artifact was a Cloudflare notice explaining the absence — which was itself the first proof the rule had matched. The re-test from an outside sender landed in the **Inbox, not Spam**. `SHOPIFY_APP_STORE_LISTING.md` and `app-store-assets/README.md` updated only for `support@`. ⚠️ **`sales@delayguardapp.com` has no routing rule** — the listing still names it, which is the same discard-silently defect R8 opened for. |
| 2026-09-20/21 | **Listing from 13 issues to 1; H3 done; R26 fixed and deployed** | Partner Dashboard + dashboard UI | Browser automation (`chrome-devtools-mcp`, own profile, logged in outside automation because Google rejects sign-in from an automated browser) reached the Partner Dashboard, which exposed the **real** submission checklist — 13 issues, several absent from this plan. **The live listing still sold SMS** in App details, a feature bullet and a category tag: the R20 fix had only ever reached the repo copy, never the form a reviewer reads. Removed; 0 mentions verified. H3's three plans were created (irreversible name + handle, confirmed with the user first). Capturing screenshots then found **R26**: the header read `0 ACTIVE` beside a tab reading `Active 8`, and cards read `Order ##1001` — both fixed TDD (v1.78), deployed, and verified in the running app. **Screenshots also surfaced PII**: order #1001 carries a real email, so its four alerts were resolved to let the seeded `Ada Lovelace` alerts lead, and the Orders tab was left out. First uploads were rejected — Shopify wants **exactly** 1600×900, not the ratio; retina captures were 3200×1800. Resized with `sips`. ⚠️ The deploy first failed `Not authorized`; `vercel link` refreshed an expired token. **Left for the user: the screencast (H8), then submit.** |
| 2026-09-22 | **Self-review before submit: launched free-only** | App Pricing + Notification Preferences | Reviewing against the App Store requirements before ticking the self-review box found that **the paid plans bought nothing**: SMS is the only paid gate, and v1.78 removed every SMS claim from the listing, leaving Free/$7/$25 with 0 described features each — an accurate-pricing failure, and a way to charge $25/mo for a feature that cannot send. Pro and Enterprise deleted (Shopify has no hide, only delete; no subscribers). The SMS toggle then pointed at a plan that no longer exists, so it is disabled with honest copy (v1.80). Also verified live: `/health`, `/legal/privacy-policy` and `/monitoring/health` all 200, the last still `degraded` on the unconfigured carrier API. |
| 2026-09-21/22 | **H8 done: screencast recorded, uploaded and in the listing — 0 issues** | Partner Dashboard + YouTube | Recorded the screencast here rather than handing it over: macOS `screencapture -v` on the external display, cropped in ffmpeg to the browser viewport alone so nothing else on screen could leak in, driven through `chrome-devtools-mcp`. Five segments (install on the new `delayguard-demo` store, settings + test alert, the three "Learn More" explainers, the alerts tab, the delivered customer email), title cards rendered with PIL, assembled with ffmpeg: **4:14**, inside the form's "aim for a 3 to 8 minute video" [S]. Uploaded **Unlisted** to YouTube (`https://youtu.be/2BX0Ztxjggo`). ⚠️ The listing field would not take the URL through a normal fill — the React input needed the native value setter plus input/change events, exactly like the dashboard inputs; the first save posted an empty field and the page still read 1 issue after reload. **Listing now verified at 0 issues, SMS mentions 0, after a reload.** Preliminary steps: automated checks **passed**, capabilities set to `embedded`. Protected customer data: the top-level reason said "email/SMS" and the Phone reason sold SMS, while the listing no longer mentions SMS at all. Reworded both; **the Phone field itself could not be removed** — `Remove` never fires a mutation and `Save` is disabled with zero reasons (3 attempts). Its reason now states the real use. **Left: the submit decision.** |
| 2026-09-21 | **R27: contact details would not save on a fresh install** | Notification Preferences → `PUT /api/merchant-settings` | Filming the screencast (H8) on a **new** dev store (`delayguard-demo`, created so the install could be filmed without uninstalling from `delayguard-dev`). Install and OAuth grant worked first time. Entering a merchant email with the optional phone empty toasted `Invalid phone format - must contain at least 10 digits` and saved nothing, because the form sends every contact field and the untouched phone arrived as `""`. A reviewer would hit it on the first settings screen. Fixed TDD (v1.79): blank email or phone now means "not provided". **Verifying that fix live exposed a second defect:** the save returned 200 but the form reloaded empty, because the client never called `GET /api/merchant-settings`; `fetchSettings` now reads it. **And the test-alert email itself showed "New estimated delivery: 2026-05-22"**, a hard-coded sample date now four months past; it is computed from today now. Also seen, not fixed: clearing the threshold field to retype it autosaves the transient `0` and toasts a validation error. |
| 2026-09-22 | Adversarial pre-launch review | Compliance + money path | Treated "submission-ready" as a claim to falsify. **R28 found and fixed (v1.81)**: two overlapping notification dispatches sent one customer the same email twice — proven with pg-mem (`Received number of calls: 2`), fixed with an atomic claim-before-send, GREEN, full gate 2,573 passing / 0 failing / 141 suites. A **false comment** in `notification-sweep.ts` was the root cause and is corrected. Two test doubles made self-consistent for `RETURNING`. **Verified PASS by probe**: compliance webhooks 401 on bad HMAC with a 404 control, `/api/settings` 401 without a token, per-shop `frame-ancestors` (varies by shop), App Bridge from the Shopify CDN, `/legal/*` 200, no committed credentials. **Open findings for the human**: `/billing/plans` still serves Pro $7 + Enterprise $25 unauthenticated after both plans were deleted; `write_orders` + `write_fulfillments` requested at install and never used; **no `app/uninstalled` webhook at all**; four third-party calls with no `AbortController`; `delayguardapp.com` has MX but no A record. **Then unblocked the browser** (the MCP profile's window was parked off-screen at `top:-941` on a disconnected display; patched `browser.window_placement`) and the dashboard falsified the headline claim: **R29 — Shopify's own embedded App Bridge check FAILS and "Submit for review" is greyed out**, caused by the root provider booting legacy `@shopify/app-bridge` v3 `createApp` on top of the CDN script (119 `APP::` types in the deployed bundle; `createApp` measured succeeding inside the admin iframe). Also new: **R30** the listing's website URL renders the app with a red "Missing Authorization header" banner; **R31** the Phone protected-data field is justified by SMS that cannot be sold or sent (Address reason is also wrong); **R32** the app's own CSP blocks the Inter stylesheet it ships. Live listing confirmed genuinely free-only (one plan, "Free"), screencast Unlisted and reachable, privacy URL 200. |
| 2026-09-22 (cont.) | Submission unblock — agent drove the dashboard | R29-R38 | **The one thing Shopify was blocking on was the app's own header.** v1.82 removed the legacy npm App Bridge and auth broke outright; the live console said `App Bridge global not present`. Root cause: `Cross-Origin-Embedder-Policy: require-corp` against a Shopify CDN that sends no CORP, so `app-bridge.js` never executed — `window.shopify` had NEVER existed and the legacy bridge had been carrying every session token. COEP/COOP removed, CORP `cross-origin` (v1.83). **Verified live**: embedded app logs `✅ App Bridge (CDN) available`, header reads Connected with 8 alerts, app-frame console 17 messages → 4. Also shipped: `app/uninstalled` + `shops.uninstalled_at` + both sweeps filtered (R33), AbortController/Promise.race on all four money-path calls (R34), free-only `/billing/plans` (R35), `vercel-build` added as an 8th quality gate after a green gate let a TS6142 deploy failure through (R36), minimum scopes (R37), landing page at `/` for non-framed requests (R30/v1.84). **Dashboard, done by the agent with the user's approval**: Phone DESELECTED from protected customer data and the Address reason corrected (R31); listing `featureList.0` changed off the carrier claim to "Automatic delay detection from Shopify fulfillment and tracking status" — both verified after save/reload. **R38**: `migrate:vercel` had run against localhost (unset `DATABASE_URL`); a SUCCESSFUL migration prints nothing because the logger sits at WARN outside development. Column landing proven by triggering the cron-sweeps workflow against the new deploy and watching it pass. Gate 2,605 passing / 0 failing / 148 suites. ⏳ **Submit still greyed** — the embedded check re-runs every 2 hours and had not yet refreshed at end of session. |
| 2026-09-22 (cont. 2) | Scope release + the guard that could not fail | R37, R39 | **`shopify app deploy` was failing for a path reason, not an auth one** — run from the repo root while `shopify.app.toml` lives in `delayguard-app/`; the CLI searches upward only. Re-run from there it listed `access_scopes` as the only `(updated)` field and released **`delayguard-2-4`**, so R37 is closed. Then the env half: `SHOPIFY_SCOPES` was still set in Vercel from 338 days ago, and the production authorize redirect still read `scope=…,write_orders,…,write_fulfillments,…` — **while `minimum-scopes.test.ts` passed**, because it only ever read the code constant the env overrides. `parseScopes` now intersects the env value with `DEFAULT_SHOPIFY_SCOPES`: narrow yes, widen never (R39, v1.85). RED first — 3 new assertions failed against the old parser, the first replaying production's exact scope string. Verified live after deploy by re-reading the same authorize redirect: `scope=read_orders,read_fulfillments,read_products,read_customers`. Gate 2,609 passing / 0 failing / 148 suites. ✅ **ALL TEN PRELIMINARY STEPS GREEN.** Shopify's embedded app check re-ran against the v1.83 COEP fix and flipped to ✅; "Fix requirement issues with your configuration" resolved to "Fixed" on the same pass. The banner now reads **"Ready to submit"** and the Submit button is live — confirmed on the rendered page, not from a DOM property (an earlier read of `button.disabled` reported enabled while Polaris still had `aria-disabled="true"`). **Submit is the user's click.** |
| 2026-09-23 | Adversarial pre-submission review | GDPR + auth path + supply chain | **Treated "Ready to submit" as a claim to falsify; four of the six Part-A claims held.** Verified live by curl, not from code: authorize redirect is exactly `scope=read_orders,read_fulfillments,read_products,read_customers` (A3 ✅); `/` cold returns a 200 landing page with `frame-ancestors 'none'` while the embedded path returns the app with per-shop `frame-ancestors` (A4 ✅); **no `Cross-Origin-Embedder-Policy` header on either path** and none anywhere in `src/` — the R29 regression has not returned (A2 server half ✅). A5 ✅ — the claim-before-send is a genuine `UPDATE … WHERE flag = FALSE RETURNING id` and is already proven against pg-mem in `notification-double-send-race.test.ts`, including a happy-path guard. A6 ✅ in code — `uninstalled_at` is added idempotently by `runMigrations` and both sweeps filter on it. **A1 and the browser half of A2 COULD NOT BE VERIFIED**: the chrome-devtools MCP profile is held by a stale `chrome-devtools-mcp` process (PID 82821, 14:51) with no remote-debugging port exposed; two attach attempts failed identically and the process was left alone because it may belong to a concurrent session. The Partner Dashboard's rendered state and `window.shopify` / `idToken()` are therefore **unconfirmed this session**. **Two real GDPR gaps found and fixed (v1.86)**, both reviewer-testable under Protected Customer Data: `shop/redact` never erased `data_access_log` (no FK, so nothing cascaded it) and `customers/redact` never erased `customer_intelligence` (the row *is* the customer — id, lifetime spend, marketing consent). RED first against pg-mem: 2 failed / 5 passed, the 5 being the over-broad-fix guards. **Auth path had a real advisory**: `jws` 3.2.2 (GHSA-869p-cjfg-cm3x, improper HMAC verification) sits directly under the `jsonwebtoken` that `shopify-session.ts` uses to verify every session token — pinned to 3.2.3. Removed `@shopify/koa-shopify-auth` + `@shopify/koa-shopify-graphql-proxy`, dead since the webhook-POST incident but still pulling `jsonwebtoken@8.5.1` (signature-validation bypass). Prod vulns 30 → 27, both auth-path advisories gone. **Two Part-B claims were FALSE and are withdrawn**: `MinimalApp` appears **0 times across all 8 built bundle files** — nothing imports it, so it does not ship; and the active `NotificationPreferences` hard-disables SMS via `SMS_AVAILABLE = false`. **`CLAUDE.md` was actively wrong** — its prescribed `UNIQUE(order_id, delay_reason)` would have swallowed every 7-day escalation re-alert forever; corrected. **Deferred with evidence, not fixed**: `@vercel/node` is an unused prod dependency carrying `undici` + `path-to-regexp` (most of the remaining 27), but moving it risks the deploy on submission day for advisories no reviewer runs — the user's call, post-submission. Also resolved: **10 commits had never been pushed**, so remote CI had seen none of the last session's work; fast-forwarded (0 behind) and all three workflows re-run. Gate 2,616 passing / 0 failing / 149 suites. **Submit remains the user's click.** |
| 2026-09-23 (cont.) | CI red on a test the local gate could not fail | v1.87 | Pushing the ten stranded commits made it visible: **remote CI was already red on `e3c03f79`**, before this session changed anything. `public/app.html` is a gitignored webpack artifact; the local gate builds before it tests so the file is always present, while CI's test job never builds, so `readAppDocument` served its missing-bundle fallback and the three framed-request assertions in `landing-page.test.ts` failed on every push. **The gate's own build step manufactured the precondition the test depended on** — structurally incapable of failing it. RED reproduced locally by moving the artifact aside (same three names), fixed by having the test write and clean up its own stand-in, verified green in both modes with the real artifact preserved. ⛔ **Not deployed.** `npm run deploy` returned `Not authorized` from Vercel although `vercel whoami` resolves and `vercel project ls` lists the project under `joonies-projects-1644afa2`; a retry with an explicit `--scope` was refused by the sandbox. The v1.86 GDPR and dependency fixes are committed and pushed but **are not live** — production still serves the pre-fix code (landing/health/monitoring all 200). Deploying is the user's step. |
| 2026-09-23 (cont. 2) | Both deferred calls decided | v1.88 | **Decided by measuring the deploy artifact, not the dependency tree.** `vercel build` writes the real lambda to `.vercel/output`; inside it `path-to-regexp` is **6.3.0 via koa-router**, outside the advisory range `4.0.0-6.2.2`, and `undici`/`tar` are absent entirely — so the vulnerable copies under `@vercel/node` never shipped. Moving `@vercel/node` to devDependencies therefore removes no reachable vulnerability, and was done anyway because 30 advisories of unreachable noise is how the two real auth-path ones hid. Safe because `typescript`/`webpack`/`ts-node`/`ts-loader` are already devDependencies and the production build uses all of them — Vercel installs devDeps despite `NODE_ENV=production`. `vercel build` verified green before and after; lambda unchanged at 23M. **Prod vulns 30 → 17, critical 1 → 0, high 17 → 10.** **`optionalAuth` deleted** (117 lines + 3 tests): exported, wired to nothing, and fails open on every error path including a DB error — it calls `await next()` in the catch. A loaded gun on an app about to be reviewed for Protected Customer Data Level 2. Suite 2,616 → 2,613, exactly the three tests removed. ⛔ **Still not deployed** — Vercel returned `Not authorized`; v1.86/87/88 are pushed but not live. |
| 2026-09-24 | Deployed, then verified the submission gate in the browser | A1/A2 closed, R40 opened | **Deployed.** Yesterday's `Not authorized` from Vercel was the Hobby daily deploy cap, not auth — it reset overnight and `npm run deploy` went straight through (production Ready, 52s build). v1.86/87/88 are now LIVE. Post-deploy probes: landing/health/monitoring/legal all 200, authorize scope still exactly the four read scopes, `shop/redact` with a bad HMAC still 401, **no COEP header** on the embedded path `[M]`. **A1 CONFIRMED** — read from the rendered page, and then from the property that fooled the previous session: all **ten** preliminary steps show green checks, the banner reads Draft / "Ready to submit", and the Submit button is `domDisabled: false`, **`aria-disabled: "false"`**, `pointerEvents: auto`, `cursor: pointer`, class `Polaris-Button--primary` with no disabled modifier `[M]`. **A2 CONFIRMED end-to-end** — the app loaded embedded in `delayguard-dev` and rendered fully: "Connected to delayguard-dev", 8 total / 4 active / 4 resolved, settings populated. The console logged `✅ App Bridge (CDN) available`, which `ShopifyProvider.readAppBridgeGlobal` emits **only** when `globalThis.shopify` exists *and* `typeof shopify.idToken === "function"`; the live alert data proves `idToken()` minted a token the backend verified through the new `jws@3.2.3` `[M/S]`. Browser access needed closing the Chrome holding the `chrome-devtools-mcp` profile — it is the automation profile, not a personal one, and cookies survive the restart. **R40 (new, deadline 2026-01-01, NOT a submission blocker)**: Dev Dashboard API health reports *"Deprecated offline token use detected in last 30 days — new expiring offline tokens are replacing tokens that grant permanent offline access"*, while *"No deprecated API calls detected"* stays green `[M]`. The app stores a permanent offline token in `shops.access_token` and uses it for every Admin API call — the cron sweeps, webhook registration and the billing plan lookup all break when the permanent grant stops working. Needs token exchange before January. **Submit is still the user's click.** |
| 2026-09-24 (cont.) | Free-tier cost visibility | v1.89 | **The 50-alert limit was never a mechanism** — it sits in three config files and nothing counts `[M: grep]`. Added `collectShopUsage`, and the finding that shaped it: **the advertised limit and the cost driver are different numbers.** The sweep scans every undelivered order inside thirty days whether or not it alerts, so the bill follows `ordersMonitored`, not alerts — a shop with 50,000 quiet orders costs compute on all 50,000 and sends nothing. Capping alerts alone would limit what the merchant sees and leave the bill untouched. **Measurement only, no enforcement**: blocking a free merchant with no paid plan to upgrade to breaks the app and earns nothing; enforcement is a one-line change once Pro exists. No schema change — both figures derive from `orders` and `delay_alerts` as they stand. RED first, then **mutation-tested**: dropping the DELIVERED filter, the month boundary, and the uninstalled-shop filter each failed exactly the assertion named for it. Logged on **both** sweep exit paths, because a no-candidate sweep is most of them on a low-traffic install and is the baseline that makes the busy numbers mean something. Gate 2,619 passing / 0 failing / 150 suites. |
| 2026-09-24 (cont. 2) | 🚀 **SUBMITTED FOR REVIEW** | — | **The app is submitted.** Status reads `Submitted`, banner *"We're assigning a reviewer to your submission"*, and a `Withdraw` button has replaced Submit `[M]`. Issues will be emailed to the submission contact. **Caught immediately before the click and fixed:** the reviewer-facing testing instructions said *"Billing: plans are Free, Pro and Enterprise via Shopify App Pricing"* — both paid plans were deleted 2026-09-22, and the same form states **"1 public plan"**. A reviewer could have falsified it in five seconds, in the exact area the requirements call out. Rewritten to name the single Free plan and to say plan gating exists in code while no paid plan is offered; saved and verified, and the gate re-checked green afterwards `[M]`. **The `sales@` risk carried since R8 was a false alarm** — it appears nowhere in the live listing. `supportEmail` and `reviewNotificationEmail` are both `support@delayguardapp.com` (routed, receipt-proven) and the submission contact is the personal address `[M]`. SHOPIFY_APP_STORE_LISTING.md line 285 describes an old draft, not the submitted state, and is stale. ⚠️ **Open decision, noticed on the post-submission screen**: App Store visibility is set so the listing *"will only be accessible to merchants who have its direct URL"* — i.e. **unlisted, no search discovery**. A `Make fully visible` button exists; whether it is reversible is **unverified**. Recommendation: stay unlisted for the first weeks — one of three detection rules fires, no real order has been processed end-to-end with tracking, cost data only starts collecting now, and App Store reviews are public and permanent. Hand the URL to a few merchants deliberately, read the new `📊 Shop usage` lines, then open discovery. |
