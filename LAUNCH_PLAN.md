...
*Rewritten 2026-08-05. **R2, R6 and R7 are all closed — the app can now receive real merchant order data for the first time.** What blocks launch is no longer detection but *delivery*: R1 (SendGrid) means a detected delay still cannot reach a buyer. Everything remaining is a human purchase or dashboard action, plus two agent-side hygiene items.*

**State at handoff:**

| | Status |
|---|---|
| **R2 — all four steps** | ✅ **closed 2026-08-05.** OAuth→`upsertShop`, 3 webhook subscriptions live on Shopify, signed `orders/updated`→Postgres, embedded dashboard on real data |
| **R7 — Protected Customer Data** | ✅ **closed 2026-08-05.** Granted after building the missing Level 2 access log; proven by a subscription Shopify had refused for six days |
| **R1 — SendGrid** | ⛔ `[HUMAN]`, **now the top blocker.** No credits, Mail-Send-only key, `delayguard.app` has no MX and no domain-auth records. A delay is detected and then goes nowhere |
| H3 / H7 `[HUMAN]` | ⛔ App Pricing plans (blocks all revenue) and ShipStation Advanced (gates third-party parcel tracking) |
...
>
> **R2, R6 and R7 are closed — do not re-verify them** (§6 records the evidence). The product can now receive real order webhooks but still cannot deliver a notification, so **R1 (SendGrid) is the top blocker** and it is a purchase decision, not code. Help the human through it: paid plan → domain-authenticate `delayguard.app` (three CNAMEs; confirm them by DNS lookup *before* they click Verify) → temporary Full Access key → create the dynamic template → hand back the `d-…` ID → verify with `/api/test-alert` → **remind them to delete the temporary key**.
>
> **While R1 is blocked on a purchase, do the agent-side work that is now unblocked:** place a real test order on `delayguard-dev.myshopify.com` and prove a *Shopify-delivered* webhook (not a synthetic one) lands in Postgres — R2 step 3 was only ever proven with a self-signed payload, because until 2026-08-05 no subscription existed. Then: B2's missing server-side OAuth query-HMAC check; R5's order-dependent flake (§6 R5 names the candidate mechanism in `resolveLegalDir()`); and the non-atomic notification dedupe in `CLAUDE.md`.
>
> **Do not** re-audit Appendix A, re-verify Appendix B, re-verify R2/R6/R7 (all closed with live evidence in §6), or start C5 (expiring tokens, post-launch — though the Dev Dashboard now flags it as *"Fix by Jan 1: calls made with deprecated offline tokens"*, so it is real, measured, and still not launch-blocking).
>
...
**Why this order:** the app can now *detect* a delay but not *tell anyone* — R1 is the difference between a demo and a product, and it gates H8's screencast, which gates submission. It is also the one blocker with a hard external dependency (a paid SendGrid plan plus DNS propagation), so starting it early costs nothing and finishing it late costs days.
