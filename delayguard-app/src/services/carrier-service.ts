...
export class CarrierService implements ICarrierService {
  ...
  async getTrackingInfo(trackingNumber: string, carrierCode: string): Promise<TrackingInfo> {
    try {
      console.log(`🔍 Fetching tracking info for ${trackingNumber} via ${carrierCode}`);
      ...
      console.error(`❌ Failed to get tracking info for ${trackingNumber}:`, error);
  ...
  async validateTrackingNumber(trackingNumber: string, carrierCode: string): Promise<boolean> {
    try {
