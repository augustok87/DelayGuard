...
export const useSettings = () => {
  ...
  const validateSettings = useCallback((settingsToValidate: AppSettings) => {
    ...
    if (
      settingsToValidate.delayThreshold < 1 ||
      settingsToValidate.delayThreshold > 30
    ) {
    ...
    if (
      !settingsToValidate.emailNotifications &&
      !settingsToValidate.smsNotifications
    ) {
      errors.push("At least one notification method must be enabled");
    }

    if (
      settingsToValidate.autoResolveDays &&
      (settingsToValidate.autoResolveDays < 1 ||
        settingsToValidate.autoResolveDays > 90)
    ) {
