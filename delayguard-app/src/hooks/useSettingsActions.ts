...
export const useSettingsActions = () => {
  ...
  }, [updateSettings, validateSettings, showErrorToast, showSaveSuccessToast, showSaveErrorToast]);
  ...
  }, [showInfoToast, showConnectionSuccessToast, showConnectionErrorToast, showErrorToast]);
  ...
  const validateCurrentSettings = useCallback((settings: any) => {
    ...
    if (!validation.isValid) {
      showWarningToast(`Settings validation failed: ${validation.errors.join(', ')}`);
    }
