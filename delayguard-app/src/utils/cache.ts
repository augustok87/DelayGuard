...
export async function cacheTrackingInfo(trackingNumber: string, carrierCode: string, data: any): Promise<void> {
  const cache = getCache('tracking');
  const key = `${trackingNumber}:${carrierCode}`;
  await cache.set(key, data, 3600); // 1 hour TTL
}

export async function getCachedTrackingInfo(trackingNumber: string, carrierCode: string): Promise<any> {
  const cache = getCache('tracking');
  const key = `${trackingNumber}:${carrierCode}`;
  return await cache.get(key);
}

export async function cacheShopSettings(shopDomain: string, settings: any): Promise<void> {
