...
function ThemeCustomizer({ onThemeChange, initialTheme = defaultTheme }: ThemeCustomizerProps) {
  ...
  const handleShareTheme = () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(`${window.location.origin}?theme=${themeUrl}`);
    } else {
  ...
  const colorPalettes = [
    ...
    { name: 'Sunset Orange', colors: ['#FF5722', '#FF9800', '#FFB74D', '#FFFFFF'] },
    { name: 'Royal Purple', colors: ['#673AB7', '#9C27B0', '#BA68C8', '#FFFFFF'] },
  ];
  ...
  const renderSelect = (label: string, property: keyof ThemeSettings, value: string, options: { label: string; value: string }[]) => (
    <div style={{ marginBottom: '16px' }}>
      <Text variant="bodyMd" as="div" style={{ marginBottom: '8px' }}>{label}</Text>
      <select
        data-testid="select"
    ...
    <div style={{ padding: '20px' }}>
      <Card title="Theme Customizer">
        <div style={{ padding: '16px' }}>
          ...
          <div data-testid="form-layout" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            ...
            {renderColorPicker('Background', 'backgroundColor', theme.backgroundColor)}
            {renderNumberInput('Border Radius', 'borderRadius', theme.borderRadius, 0, 20)}
            ...
            {renderNumberInput('Animation Speed', 'animationSpeed', theme.animationSpeed, 100, 1000)}
            {renderCheckbox('Dark Mode', 'darkMode', theme.darkMode)}
            {renderCheckbox('Compact Mode', 'compactMode', theme.compactMode || false)}
            {renderCheckbox('Show Animations', 'showAnimations', theme.showAnimations || true)}
          </div>
          ...
          <div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <Button size="sm" variant="secondary" onClick={handleColorSuggestions}>
              Color Suggestions
            </Button>
      ...
      {showPreview && (
        ...
        >
          <div style={{ padding: '20px', backgroundColor: theme.backgroundColor, color: theme.textColor }}>
            <Text variant="headingMd" as="h3" style={{ color: theme.primaryColor }}>
