...
function generateRecommendations(
  ...
  if (unhealthyChecks.length > 0) {
    recommendations.push(
      `Critical: ${unhealthyChecks.map((c) => c.name).join(", ")} are unhealthy and need immediate attention`,
    );
  }
  ...
  if (degradedChecks.length > 0) {
    recommendations.push(
      `Warning: ${degradedChecks.map((c) => c.name).join(", ")} are degraded and should be monitored closely`,
    );
  }
  ...
  if (status.metrics && status.metrics.memory.percentage > 80) {
    recommendations.push(
      `High memory usage detected (${status.metrics.memory.percentage}%). Consider scaling or optimizing memory usage`,
    );
  }
  ...
  if (status.metrics && status.metrics.application.responseTime.average > 500) {
    recommendations.push(
      `Slow response times detected (${status.metrics.application.responseTime.average}ms). Consider performance optimization`,
    );
  }
  ...
  if (status.metrics && status.metrics.application.requests.successRate < 99) {
    recommendations.push(
      `Low success rate detected (${status.metrics.application.requests.successRate}%). Investigate error causes`,
    );
  }
