...
router.get('/analytics', async(ctx) => {
  try {
    ...
    const metrics = await analyticsService.getAnalyticsMetrics(shopId, timeRange);
...
router.get('/analytics/export', async(ctx) => {
  try {
    ...
    if (format === 'csv') {
      ...
      ctx.set('Content-Disposition', `attachment; filename="delayguard-analytics-${timeRange}.csv"`);
      ctx.body = csvData;
    } else {
      ctx.body = {
        success: true,
...
function convertToCSV(metrics: {
  ...
  const rows = [
    ...
    ['Email Success Rate', `${metrics.notificationSuccessRate.email}%`, 'Notifications'],
    ['SMS Success Rate', `${metrics.notificationSuccessRate.sms}%`, 'Notifications'],
