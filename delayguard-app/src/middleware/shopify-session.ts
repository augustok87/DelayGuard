...
export const getShopDomain = (ctx: Context): string => {
  ...
  if (!shopDomain) {
    throw new Error(
      "Shop domain not found in context. Ensure requireAuth middleware is used.",
    );
  }
  return shopDomain;
};
...
export const optionalAuth = async (ctx: Context, next: Next) => {
  ...
  if (!authHeader) {
    logger.debug(
      "Optional auth: No auth header, continuing without authentication",
      {
        path: ctx.path,
      },
    );
  ...
  if (!token) {
    logger.debug(
      "Optional auth: No token found, continuing without authentication",
      {
        path: ctx.path,
      },
    );
  ...
  try {
    ...
    if (!apiSecret || !apiKey) {
      logger.warn(
        "Optional auth: Missing API credentials, continuing without authentication",
      );
      await next();
      return;
    }
    ...
    if (!decoded.dest || !decoded.iss) {
      logger.debug(
        "Optional auth: Invalid token claims, continuing without authentication",
      );
      await next();
      return;
    }
    ...
    if (shopResult.length === 0) {
      logger.debug(
        "Optional auth: Shop not found, continuing without authentication",
      );
      await next();
      return;
    }
    ...
    logger.debug(
      "Optional auth: Error validating token, continuing without authentication",
      {
        path: ctx.path,
