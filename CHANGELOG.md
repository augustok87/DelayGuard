...
**Test Results**: 2,400 passing (+5), 25 skipped, 0 failing (full suite, verified this session from `delayguard-app/`; the lone `input-sanitization.test.ts:405` performance-budget flake only trips under concurrent CPU load and passes in isolation). New coverage in `src/services/test-alert-service.test.ts` (15 → 20 cases): SMS suppressed when the shop's live plan excludes SMS despite `sms_enabled = true` (stale-flag billing leak); SMS suppressed on free even when `channels: ['sms']` is explicitly requested; SMS dispatched on the enterprise plan; plan lookup skipped when SMS would not fire anyway (email-only request, and `sms_enabled = false`).

**Problem (billing leak)**: `TestAlertService.dispatchTestAlert` gated the SMS channel only on `app_settings.sms_enabled` + phone presence, and the route ([api.ts](delayguard-app/src/routes/api.ts)) only ran the Pro-plan gate when the client *explicitly* named the `sms` channel. A shop downgraded from Pro that still carried a stale `sms_enabled = true` could POST `/api/test-alert {"delayType":"warehouse"}` (no `channels`) and fire a real, billable Twilio SMS off-plan. The production dispatcher ([notification.ts:182-198](delayguard-app/src/queue/processors/notification.ts#L182)) already guards this exact stale-flag case with a live-plan check; test-alert diverged. Surfaced by the read-only robustness audit (`/audit delayguard-app/src`).

**Fix**: `dispatchTestAlert` now mirrors `notification.ts` — the SMS branch resolves `billingService.getCurrentPlan()` + `isSmsAllowed()` before dispatch (fails closed to `"free"`, so a Shopify outage suppresses SMS rather than leaking it), and only resolves the plan when SMS would otherwise fire (no GraphQL call on email-only alerts). Defense-in-depth behind the route's existing `ensureSmsPlan` gate.

**Dev infrastructure (commit-gate wiring)**: the husky pre-commit gate was inert — `core.hooksPath` was unset, husky was not installed, and `.husky/_/husky.sh` (sourced by the hook) did not exist, so `delayguard-app/.husky/pre-commit` was never invoked by git (repo root is one level above the app dir). Rewrote the hook to be self-contained (no husky runtime; `cd`s into the app dir; runs `node scripts/quality-gates.js`) and added a dependency-free `prepare` script (`git -C .. config core.hooksPath delayguard-app/.husky`) so `npm install` re-establishes the wiring. Verified git now executes the hook via `git hook run pre-commit`. CI (`.github/workflows/quality-gates.yml`) already gated `main`/`develop`; this restores the *local* gate.

**Docs**: added a DelayGuard-specific "Money-path & third-party invariants" section to [CLAUDE.md](CLAUDE.md) (live-plan gating, notification idempotency, third-party timeout + BullMQ retry, validate-before-Postgres), including two ⚠️ known-gap notes for the deferred items below.

**Deferred (verified-real robustness items, NOT hasty-patched — each needs a designed change + tests)**:
- **Non-atomic check-then-send** in `notification.ts`: `email_sent`/`sms_sent` are read via `SELECT` then written after send, so overlapping Vercel-cron ticks can double-send. Fix requires claim-before-send (`UPDATE … WHERE email_sent = FALSE RETURNING`) with careful failure-unclaim semantics (naive application trades double-send for lost-notification-on-failure).
- **`delay_alerts` has no unique constraint** backing `ON CONFLICT DO NOTHING` ([delay-check.ts:231](delayguard-app/src/queue/processors/delay-check.ts#L231)) — the guard is a silent no-op. Fix requires a forward-compatible migration (dedupe existing rows → `UNIQUE(order_id, delay_reason)` → point `ON CONFLICT` at it); the project has no rollback tooling, so the migration must not fail on existing duplicates.
