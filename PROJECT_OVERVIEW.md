...
**Last Updated**: August 26, 2026 (v1.67–v1.69 — R17 fixed and proven in production, R19 found and fixed, R18 2/3, R11 closed)
...
| **Test Success** | **100%** | 2,474 passing of 2,499, 25 skipped, 0 failing across 135 suites as of 2026-08-26 (+28 for R17/R19/R18/R11). ⚠️ §6 R5 is **open** — wall-clock assertions failing under machine load; three full runs on 2026-08-26 gave 1 failure, 1 *different* failure, then 0, each green in isolation. Never run the suite in two sessions at once. **New:** `src/tests/helpers/pg-mem-schema.ts` runs the production `runMigrations()` against a real in-memory SQL engine — use it for any assertion about what a statement *did*, since `__mocks__/pg.js` reports `rowCount: 1` for every `UPDATE` |
